...
// -- hue wraparound: a box that straddles red must catch both sides --
{
  const data = new Uint8ClampedArray([
    255, 0, 4, 255, // near-red (low hue, just above 0)
    0, 255, 90, 255, // cyan -- must NOT match
    255, 4, 0, 255, // near-red the other way (just below 180)
  ]);
  ...
  assert(mask[0] === 255 && mask[2] === 255, "both sides of the wrap must match");
...
// -- connected components + min-area filtering --
{
  ...
  assert(all.length === 2, `expected 2 blobs before filtering, got ${all.length}`);

  const filtered = cv.findBlobs(mask, w, h, 5);
  assert(filtered.length === 1, `min-area should drop the speck, got ${filtered.length}`);
  assert(filtered[0].area === 16, `blob area should be 16, got ${filtered[0].area}`);
  assert(
    Math.abs(filtered[0].cx - 2.5) < 1e-9 && Math.abs(filtered[0].cy - 2.5) < 1e-9,
...
// -- selectBlob: continuity beats size, and an impossible jump is rejected --
{
  ...
  assert(cv.selectBlob(blobs, null).cx === 100, "no previous position -> largest wins");
  assert(
...
// -- morphology: a lone pixel dies under a 3x3 open; a solid block survives --
{
  ...
  assert(opened.every((v) => v === 0), "a lone pixel must be erased by a 3x3 open");

  const block = new Uint8Array(w * h);
  for (let y = 2; y < 7; y++) for (let x = 2; x < 7; x++) block[y * w + x] = 255;
  const openedBlock = cv.cleanMask(block, w, h, { open_ksize: 3, close_ksize: 0 });
  assert(openedBlock[4 * w + 4] === 255, "the center of a solid block must survive opening");
}
...
// -- clicking the magnet must re-detect the very pixel it was sampled from --
{
  ...
  const color = cv.sampleColorRange({ width: w, height: h, data }, 10, 10, { radius: 5 });
  const { mask } = cv.buildMask({ width: w, height: h, data }, color, {});
  assert(mask[10 * w + 10] === 255, "the sampled colour must re-detect its own source pixel");
}
